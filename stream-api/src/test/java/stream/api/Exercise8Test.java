...
public class Exercise8Test extends ClassicOnlineStore {
    ...
    public void itemsNotOnSale() {
        ...
        List<String> itemListOnSale = shopStream.flatMap(shop -> shop.getItemList().stream())
                .map(Item::getName).distinct().collect(Collectors.toList());

        Set<String> itemSetNotOnSale = customerStream.flatMap(customer -> customer.getWantToBuy().stream())
                .map(Item::getName).filter(t -> itemListOnSale.stream().noneMatch(t::equals))
    ...
    public void havingEnoughMoney() {
        ...
        Predicate<Customer> havingEnoughMoney =  customer ->
                customer.getBudget() >= customer.getWantToBuy().stream().mapToInt(
                        wantByItem -> onSale.stream()
                                    .filter(saleItem -> wantByItem.getName().equals(saleItem.getName()))
